...
$oldTotal = 0;
$oldSkus = array();
foreach ($data->oldSkus as $oldSkuData) {
    $oldSku = $skusRepository->findByCode($oldSkuData->code);
    if (count($oldSku) > 0) {
        if (!$oldSku[0]->isProperPrice($oldSkuData->price)){
            $error = array('field' => 'skus', 'code' => 'wrong_price', 'value' => $oldSkuData->code);
            array_push($errors, $error);
            break;
        }
        $oldTotal += $oldSkuData->price * $oldSkuData->quantity;
        array_push($oldSkus, $oldSku);
    } else {
          $error = array('field' => 'skus', 'code' => 'not_found', 'value' => $oldSkuData->code);
          array_push($errors, $error);
          break;
    }
}
...
$data->netoldTotal = $oldTotal;
$data->grandoldTotal = $oldTotal;

$id = $invoicesRepository->returnSalesInvoice($data, $skus, $oldSkus, $currentUser->getId());
